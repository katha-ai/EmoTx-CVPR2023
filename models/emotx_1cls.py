...
class FramePositionalEncoding(nn.Module):
    ...
    def forward(self, feat, frames):
        ...
        return self.dropout(out) # out


class Emotx_1CLS(nn.Module):
    """
    Varient of Emotx model with 1-CLS responsible for classifying 'k' emotions for scene and characters.
    """
    def __init__(self, num_labels, num_pos_embeddings, scene_feat_dim, char_feat_dim, srt_feat_dim, num_enc_layers=4, num_chars=8, max_individual_tokens=100, hidden_dim=512):
        ...
        Args:
            ...
            srt_feat_dim (int): Dimension of srt features.
            num_enc_layers (int): Number of transformer encoder layers.
            num_chars (int): Number of characters in a scene.
        ...
        """
        super(Emotx_1CLS, self).__init__()
        ...
        self.cls_token = nn.Parameter(torch.randn(self.hidden_dim))
        ...
        self.logits = nn.Linear(self.hidden_dim, num_labels)
        self.mask_vec = torch.zeros((1,1))
        self.seq_len = (self.num_chars+1)*max_individual_tokens + self.num_chars+1 + max_individual_tokens
        self.src_mask = torch.zeros((self.seq_len, self.seq_len)).to(torch.bool)

    def append_tokens(self, batch_size, feat):
        """
        Append CLS tokens to the beggining of the feature vectors.

        Args:
            batch_size (int): Batch size.
            feat (torch.Tensor): Feature vectors. Tensor of shape (batch_size, num_frames+1, feat_dim)
        """
        cls_emb = self.cls_token.to(feat.device).repeat(batch_size, 1, 1)
    ...
    def forward(self, feats, masks):
        ...
        target_masks = [char_target_mask, scene_target_mask]
        ...
        # Append 1-CLS token around scene features
        scene_feat = self.append_tokens(batch_size, scene_feat)

        # Append 1-CLS token around character features
        char_feats = torch.stack([self.append_tokens(batch_size, char_feats[:,i,:,:]) for i in range(self.num_chars)], dim=1)
        ...
        feat_mask = self.mask_vec.repeat(batch_size, 1).to(scene_feat.device)
        ...
        cls_ndxs = torch.cumsum(torch.tensor([0]+[scene_feat.shape[1]]+[char_feats.shape[-2]]*(self.num_chars-1)), dim=0).to(char_feats.device)
        ...
        # Filter the embeddings coresponding to CLS token.
        cls_heads = out[:, cls_ndxs, :]

        # Pass the CLS embeddings to the logits layer. The CLS embedding and logits layer are responsible for multi-label emotion classification here.
        pred = self.logits(cls_heads)
        ...
        filtered_preds = [pred.flatten()[mask] for mask in target_masks]
